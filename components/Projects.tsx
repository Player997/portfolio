    ...
    <section id="projects" className="py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="reveal flex flex-col md:flex-row justify-between items-end mb-12">
          <div>
            <h2 className="text-3xl md:text-4xl font-bold text-text mb-2">Featured Projects</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {PROJECTS.map((project, index) => (
            <div 
              key={index}
              className={`reveal reveal-delay-${(index % 3) * 100} group relative glass-panel rounded-2xl overflow-hidden hover:shadow-xl hover:-translate-y-2 border border-black/5 dark:border-white/5 hover:border-primary/30 flex flex-col transition-all duration-500`}
            >
              {/* Image / Preview Area with Code Editor Look */}
              <div className="h-56 bg-surface relative flex flex-col overflow-hidden border-b border-black/5 dark:border-white/5">
                 {/* Window Controls */}
                 <div className="h-8 bg-black/5 dark:bg-white/5 flex items-center px-4 gap-2 border-b border-black/5 dark:border-white/5">
                   <div className="w-3 h-3 rounded-full bg-red-500/50"></div>
                   <div className="w-3 h-3 rounded-full bg-yellow-500/50"></div>
                   <div className="w-3 h-3 rounded-full bg-green-500/50"></div>
                   <div className="ml-auto text-[10px] text-muted font-mono">App.tsx</div>
                 </div>
                 
                 {/* Code Content */}
                 <div className="flex-1 p-4 font-mono text-xs text-text/60 overflow-hidden relative group-hover:text-text/80 transition-colors">
                    <div className="absolute inset-0 bg-gradient-to-t from-surface to-transparent z-10"></div>
                    <div><span className="text-purple-500 dark:text-purple-400">const</span> <span className="text-blue-500 dark:text-blue-400">{project.title.split(' ')[0]}</span> = () ={'>'} {'{'}</div>
                    <div className="pl-4"><span className="text-purple-500 dark:text-purple-400">return</span> (</div>
                    <div className="pl-8 text-green-600 dark:text-green-400">'Transforming Ideas'</div>
              ...
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-2">
                   <h3 className="text-xl font-bold text-text group-hover:text-primary transition-colors">{project.title}</h3>
                   <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-muted uppercase tracking-wider">
                      {project.category}
                   </span>
                </div>
                ...
                <div className="mt-auto">
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tech.map((t) => (
                      <span key={t} className="text-xs font-medium text-muted bg-surface border border-black/5 dark:border-white/5 px-2 py-1 rounded hover:text-text hover:border-primary/30 transition-colors">
                        {t}
                      </span>
                    ))}
                  </div>
